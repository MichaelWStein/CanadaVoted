# This program creates a datafile with key information for all electoral Districts.
# For details, please look at the jupyter lab file (for one electoral district): DataExtractionOneDistrict.ipynb 
...
# Create a list of urls to call (for 2016 electoral districts)

def getdata(url):

    data = urllib.request.urlopen(url).read()

    #Need to remove the 'head' of the Statscan data, then it can be jsonified.
    data1 = data[2:]
...
# Function for loading the data for each electoral district and transforming it.
...
def dist_data(url):
    ...
    req_info = [0, 5, 38, 57, 114, 115, 116, 673, 859, 1140, 1141]
    for req in req_info:
...
# For review also create a *.csv file
el_dist_df.to_csv("StatsCan_2016_ElecDist.csv")
